...
export default function LawComponent({ law, index }) {
  ...
  const openPdfFile = () => {
    const match = law.date_jarida.match(/\d{4}/);
  ...
  return (
    <div className={`flex flex-col justify-around md:flex-row rounded-md shadow-md ${backgroundColor} bg-opacity-15 py-4`}>
       ...
       <div className='flex flex-col text-right'>
         <p className='text-Dark_Blue font-cairo'> {law.num_jarida }الجريدة الرسمية رقم {law.page_jarida} ,الصفحة </p>
         <p className='text-Dark_Blue font-cairo'>المؤرخة في {law.date_jarida}</p>
