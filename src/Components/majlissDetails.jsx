...
export default function MajlissDetails(props) {
    const [majlissDetails, setMajlissDetails] = useState(null);

    useEffect(() => {
        const fetchMajlissDetails = async () => {
            try {
                const response = await fetch(`http://127.0.0.1:8000/DetailsqrarMajliss/${props.id}`);
                const data = await response.json();
                setMajlissDetails(data);
                console.log(majlissDetails.classe)
            } catch (error) {
                console.error('Erreur lors de la récupération des détails du qrarMajliss : ', error);
            }
        };
        ...
        <div style={{ marginBottom: "5%" }}>
            {majlissDetails ? (
                <div className="flex flex-col mt-20 mx-20 mb-8" style={{ border: "5px solid #0C6291", borderRadius: '15px', opacity: 0.69 }}>
                    <div className="flex flex-col text-center md:flex-row justify-content-center mt-10 mb-4">
                        <h2 className="text-xl" style={{ margin: 'auto', opacity: '1', fontFamily: "Cairo", fontSize: "32px", fontWeight: "bold" }}>
                            <span style={{ color: "#000000" }}>{majlissDetails.raqmQarar}</span> <span style={{ color: "#0C6291" }}>تفاصيل القرار</span> 
                        </h2>
                    </div>
                    <div className="flex flex-col text-center md:flex-row justify-content-center mb-1">
                        <p className="mb-2" style={{ margin: 'auto', color: "#9FC2CC", opacity: '1', fontFamily: "Cairo", fontSize: "20px", fontWeight: "semibold" }}>المؤرخ في {formatDate(majlissDetails.raqmQarar)}</p>
                    </div>
                    ...
                    <div className="flex flex-row justify-content-around mb-12 mt-16" style={{ alignItems: "flex-start"}}>
                        <div className="flex flex-col my-2 mx-10 mb-12" style={{ opacity: "1", width: "40%" }}>
                            <p className="mb-2 text-center" style={{ margin: 'auto', color: "#0C6291", opacity: '1', fontFamily: "Cairo", fontSize: "20px", fontWeight: "semibold" }}>المبدأ</p>
                            <p className="flex flex-col my-7 mx-7 text-right">
                                {majlissDetails.principe}
                            </p>
                        </div>
                        <hr style={{ border: "1px solid #9FC2CC", width: "1px", margin: "auto", height: "200px" }} />
                        <div className="flex flex-col my-2 mx-10 mb-12" style={{ opacity: "1", width: "40%" }}>
                            <p className="mb-2 text-center" style={{ margin: 'auto', color: "#0C6291", opacity: '1', fontFamily: "Cairo", fontSize: "20px", fontWeight: "semibold" }}>التكييف</p>
                            <p className="flex flex-col my-7 mx-7 text-right">
                                {majlissDetails.takyif}
                            </p>
                        </div>
                        <hr style={{ border: "1px solid #9FC2CC", width: "1px", margin: "auto", height: "200px" }} />
                        <div className="flex flex-col my-2 mx-10 mb-12" style={{ opacity: "1", width: "40%" }}>
                        ...
                        <div className="flex flex-row justify-content-around mb-12 mt-16" style={{ alignItems: "flex-start"}}>
                        <hr style={{ border: "1px solid #9FC2CC", width: "1px", margin: "auto", height: "200px" }} />
                        {majlissDetails.classe === "NaN" || majlissDetails.classe === null ? (
                            
                        <div className="flex flex-col my-2 mx-10 mb-12" style={{ opacity: "1", width: "40%" }}>
                                <p className="mb-2 text-center" style={{ margin: 'auto', color: "#0C6291", opacity: '1', fontFamily: "Cairo", fontSize: "20px", fontWeight: "semibold" }}>القسم</p>
                                <p className="flex flex-col my-7 mx-7 text-right">لا يوجد قسم</p>
                            </div>
                        ) : (
                            <div className="flex flex-col my-2 mx-10 mb-12" style={{ opacity: "1", width: "40%" }}>
                                <p className="mb-2 text-center" style={{ margin: 'auto', color: "#0C6291", opacity: '1', fontFamily: "Cairo", fontSize: "20px", fontWeight: "semibold" }}>القسم</p>
                                <p className="flex flex-col my-7 mx-7 text-right">
                                    {majlissDetails.classe}
                                </p>
                            </div>
                        )}
                        
                        <hr style={{ border: "1px solid #9FC2CC", width: "1px", margin: "auto", height: "200px" }} />
                        {majlissDetails.chambre === null ? (
                            <div className="flex flex-col my-2 mx-10 mb-12" style={{ opacity: "1", width: "40%" }}>
                                <p className="mb-2 text-center" style={{ margin: 'auto', color: "#0C6291", opacity: '1', fontFamily: "Cairo", fontSize: "20px", fontWeight: "semibold" }}>الغرفة</p>
                                <p className="flex flex-col my-7 mx-7 text-right">لا يوجد غرفة</p>
                            </div>
                        ) : (
                            <div className="flex flex-col my-2 mx-10 mb-12" style={{ opacity: "1", width: "40%" }}>
                                <p className="mb-2 text-center" style={{ margin: 'auto', color: "#0C6291", opacity: '1', fontFamily: "Cairo", fontSize: "20px", fontWeight: "semibold" }}>الغرفة</p>
                                <p className="flex flex-col my-7 mx-7 text-right">
                                    {majlissDetails.chambre}
                                </p>
                            </div>
                        )}
